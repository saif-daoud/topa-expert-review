...
async function dbGetClaimByCodeHash(env: Env, codeHash: string): Promise<ChunkClaimRow | null> {
  const row = await env.DB
    .prepare("SELECT chunk_id, code_hash, claimed_at, updated_at FROM chunk_claims WHERE code_hash = ? LIMIT 1")
    .bind(codeHash)
    .first<ChunkClaimRow>();
  return row ?? null;
}
...
async function dbClaimChunk(env: Env, chunkId: number, codeHash: string) {
  const now = new Date().toISOString();
// Enforce: one chunk per access code
const existingForCode = await dbGetClaimByCodeHash(env, codeHash);
if (existingForCode && existingForCode.chunk_id !== chunkId) {
  const e: any = new Error("This access code is already assigned to another chunk");
  e.status = 409;
  e.assigned_chunk = existingForCode.chunk_id;
  throw e;
}
  ...
  // Likely unique constraint (code already assigned to another chunk)
  const again = await dbGetClaimByCodeHash(env, codeHash);
  if (again && again.chunk_id !== chunkId) {
    const e: any = new Error("This access code is already assigned to another chunk");
    e.status = 409;
    e.assigned_chunk = again.chunk_id;
    throw e;
  }
  throw err;
}
  ...
  async fetch(req: Request, env: Env) {
    ...
    try {
      ...
      if (path.endsWith("/api/start")) {
        ...
        const assigned = await dbGetClaimByCodeHash(env, codeHash);

        return new Response(JSON.stringify({ ok: true, token, participant_id, assigned_chunk: assigned?.chunk_id ?? null }), { status: 200, headers });
      }
